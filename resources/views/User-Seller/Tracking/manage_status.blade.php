...
if(isset($_SESSION['email'])){
    ...
    $s = $conn->query("SELECT * FROM buyout WHERE id='$id' AND date = '$date' AND idSeller = '".$_SESSION['Id']."'");
    $r = mysqli_fetch_array($s);

    if(isset($modify)){
        $conn->query("UPDATE buyout SET status = '$status' WHERE id = '$id'");
        redir("/user-seller-trackings");
    }
}
?>
<!DOCTYPE html>
      ...
      <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-body">
              <form method="post" action="{{ route('change_status') }}" role="form">
                <input type="hidden" name="_token" value="{{ csrf_token()}}">
                <div class="form-group">
                  <input type="hidden" name="id" value="<?=$id = clear($id);?>">
                  <select class="form-control" style='font-size: .8rem; border-radius: 10rem; height:3rem; margin-bottom:15px;' name="status">
                    <option <?php if($r['status'] == 0){echo "selected";}?> value="0">Starting</option>
                    <option <?php if($r['status'] == 1){echo "selected";}?> value="1">Preparing</option>
                    <option <?php if($r['status'] == 2){echo "selected";}?> value="2">Sending</option>
                    <option <?php if($r['status'] == 3){echo "selected";}?> value="3">Delivered</option>
                  </select>
                </div>
                <div class="form-group">
                  <input class="btn btn-primary" type="submit" value="set status" name="modify">
                </div>
              </form>
            </div>
          </div>
