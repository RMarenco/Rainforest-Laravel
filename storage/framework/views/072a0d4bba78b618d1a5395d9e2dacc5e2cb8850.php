...
$Db=Database::connect();
	...
	if(isset($add) && isset($quan)){
      ...
      if($pq >= $quan){
        ...
        alert("This product only has " .$pq. " stocks.");
      } 				
    }else{			
      $f = $Db->query("UPDATE products SET quantity = quantity - $quan WHERE idProduct = '$idp' AND quantity >= '$quan'"); 	
      if($pq >= $quan){
        ...
        <div id="collapseUtilities" class="collapse" aria-labelledby="headingUtilities" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
            ...
            <a class="collapse-item" href="<?php echo e(route('logout')); ?>" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
              <?php echo e(__('messages.log-out')); ?><?php echo e(__('messages.log-out')); ?>
      ...
      <div class="container-fluid">
        ...
        <div class="view">
          <div class="grid"> <!-- GRID PRODUCT -->
                   ...
                   <div class="row no-gutters align-items-center ">
                     <div class="col mr-2" style="font-family:'Abel',sans-serif;text-align:center;"><br>
                       ...
                       if($Producto['discount']>0){
                        ...
                        echo "<br/>";
                        echo '<span class="product__price highlight h5 mb-0 font-weight-bold text-gray-800">$'.number_format($Producto['price'], 2, '.', '').'</span>';
                      }
                      ?>
            ...
            <div id="modal_<?=$Producto['idProduct']?>" class="modale">
              ...
              <div class="modale-content" id="ctn_<?=$Producto['idProduct']?>">
                ...
                <div class="modale-footer">
                  <h7 style="float: right; position: relative; top: 25%;"><?=$Producto['product']?></h7>
            ...
            <div id="add_<?=$Producto['idProduct']?>" class="modale">
              <!-- Modal content -->
              <div class="modale-content" id="addCart_<?=$Producto['idProduct']?>">

                <div class="modale-header">

                  <h5 style="float: left!important;">Products added to the cart</h5>
                </div>
                <div class="modal-body">
                  <h3><?=$Producto['product']?> added to the cart</h3>
                  <h7 id="q_<?=$Producto['idProduct']?>"></h7>              
                </div>
                <div class="modale-footer">
                  <h5 style="float: right; position: relative; top: 25%;"><?=$Producto['product']?></h5>
                  <button class="action action--button action--buy btn btn-5 btn-5a icon-cart button" id="btn_<?=$Producto['idProduct']?>" style="background: #dcdcdc; color: #000; width: 150px; height: 40px; font-size: 14px; margin:0"><span class="action__text">Add to cart</span></button>
                </div>
              </div>    
            </div>
            <script>                           
                      ...
                      var add_<?=$Producto['idProduct']?> = document.getElementById('add_<?=$Producto['idProduct']?>');
                      var addCart_<?=$Producto['idProduct']?> = document.getElementById('addCart_<?=$Producto['idProduct']?>');
                      ...
                      function quantity<?=$Producto['idProduct']?>(idp){
                        ...
                        if(q < (<?=$Producto['quantity']?> + 1) && q>0){
                          modal<?=$Producto['idProduct']?>.classList.remove('show');
                          modal_content<?=$Producto['idProduct']?>.classList.remove('show');
                          modal<?=$Producto['idProduct']?>.style.opacity = "0";

                          /*add_<?=$Producto['idProduct']?>.classList.add('show');
                          add_<?=$Producto['idProduct']?>.style.opacity = "1";
                          addCart_<?=$Producto['idProduct']?>.classList.add('show');*/

                          swal({title:"ADDED",
                            text: "Product added succesfully",
                            icon: "success"});
                          $(".swal-button").click(function(){window.location="?p=products&add="+idp+"&quan="+q; });

                          /*document.getElementById("q_<?=$Producto['idProduct']?>").innerHTML = q;
                          document.getElementById("btn_<?=$Producto['idProduct']?>").onclick = function(){
                            window.location="?p=products&add="+idp+"&quan="+q;}*/
                            window.addEventListener("click", function(event) {                              
                              if(event.target == add_<?=$Producto['idProduct']?>){
                                window.location="?p=products&add="+idp+"&quan="+q;
                              }
                            });                                                       
                          }
...
</html><?php /**PATH D:\rainforest\resources\views/index.blade.php ENDPATH**/ ?>